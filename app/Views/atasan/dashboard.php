...
<div class="min-h-screen bg-slate-50 px-6 py-8 font-sans text-slate-800">
            ...
            <div x-data="{ openProfile: false }" class="relative">
                <button @click="openProfile = !openProfile"
                    ...
                    <div class="hidden sm:block text-left">
                        <p class="text-xs font-bold text-slate-700 leading-tight tracking-wide uppercase"><?= esc($atasan['nama']) ?></p>
                        <p class="text-[10px] text-slate-400 font-medium tracking-wider">ATASAN POLBAN</p>
                    </div>
                ...
                <div x-show="openProfile" 
                    ...
                    <div class="p-2 space-y-1">
                        <a href="<?= base_url('atasan/profile') ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-700 transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
        ...
        <div class="lg:col-span-2 relative bg-white rounded-2xl p-8 shadow-sm border border-slate-200 card-hover transition-all duration-300 group overflow-hidden">
            <div class="absolute -right-6 -top-6 w-32 h-32 bg-indigo-50 rounded-full opacity-50 transition-transform group-hover:scale-110"></div>
            
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-6 text-indigo-600 font-bold text-xs uppercase tracking-[0.2em]">
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <div class="flex items-baseline gap-2">
                            ...
                            <span class="text-slate-400 font-semibold text-lg">Indikator PIC</span>
                        </div>
                        <p class="text-slate-500 mt-2 max-w-sm">Daftar indikator kinerja yang berada di bawah tanggung jawab langsung Anda sebagai PIC.</p>
                    </div>
                    
                    <a href="<?= site_url('atasan/task') ?>" class="w-full md:w-auto px-6 py-3 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-indigo-600 transition-colors flex items-center justify-center gap-2 group/btn">
                        Isi Pengukuran
        ...
        <div class="bg-amber-500 rounded-2xl p-8 text-white shadow-lg shadow-amber-100 relative overflow-hidden">
            <svg class="absolute bottom-[-20%] right-[-10%] w-40 h-40 text-amber-400 opacity-50" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
                <path fill-rule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clip-rule="evenodd" />
            </svg>
            <div class="relative z-10">
                <h4 class="text-amber-100 text-xs font-bold uppercase tracking-widest mb-4">Status System</h4>
                ...
                <div class="pt-4 border-t border-amber-400/50">
                    <div class="flex items-center justify-between text-sm">
                        <span>Integritas Data</span>
                        <span class="font-bold italic">Active</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <a href="<?= site_url('atasan/task') ?>"
       class="flex flex-col p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-blue-400 hover:shadow-md transition-all group">
        ...
        <span class="text-[10px] text-slate-400 uppercase tracking-wider font-semibold mt-1">Input Realisasi PIC</span>
    </a>

    <a href="<?= site_url('atasan/grafik') ?>"
       class="flex flex-col p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-emerald-400 hover:shadow-md transition-all group">
        ...
        <span class="block text-sm font-bold text-slate-700">Pantau Capaian</span>
        <span class="text-[10px] text-slate-400 uppercase tracking-wider font-semibold mt-1">Statistik Bidang</span>
    </a>

    <a href="<?= site_url('atasan/profile') ?>"
    ...
    <a href="<?= site_url('atasan/activity-log') ?>"
       class="flex flex-col p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-slate-400 hover:shadow-md transition-all group">
        <div class="p-3 w-fit rounded-xl bg-white text-slate-600 shadow-sm group-hover:bg-slate-600 group-hover:text-white transition-all mb-4">
            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
        </div>
        <span class="block text-sm font-bold text-slate-700">Log Aktivitas</span>
        <span class="text-[10px] text-slate-400 uppercase tracking-wider font-semibold mt-1">Riwayat Sistem</span>
    </a>
    ...
    <div class="mt-16 pt-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="flex gap-6">
            <span class="text-[10px] text-slate-300 uppercase tracking-widest font-bold italic">Verifikasi</span>
            <span class="text-[10px] text-slate-300 uppercase tracking-widest font-bold italic">Akurasi</span>
            <span class="text-[10px] text-slate-300 uppercase tracking-widest font-bold italic">Kualitas</span>
        </div>
    </div>
